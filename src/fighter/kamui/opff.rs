...
unsafe extern "C" fn kamui_galeforce_attack(fighter: &mut L2CFighterCommon) {
    ...
    if VarModule::is_flag(fighter.module_accessor, commons::instance::flag::GALEFORCE_ATTACK_ON) {
        if VarModule::get_int(fighter.module_accessor, commons::instance::int::GA_DURATION) > 0 {
            VarModule::sub_int(fighter.module_accessor, commons::instance::int::GA_DURATION, 1);
            if [hash40("special_s_wall_attack_b"), hash40("special_s_wall_attack_f")].contains(&curr_motion_kind) {
                MotionModule::set_rate(fighter.module_accessor, 1.4);
            }
            if VisibilityModule::get_int64(fighter.module_accessor, hash40("dragon") as i64) != VisibilityModule::get_int64(fighter.module_accessor, hash40("dragon_horn") as i64) {
