...
unsafe extern "C" fn stepjump(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
...
pub fn install(agent: &mut smashline::Agent) {
    ...
    agent.game_acmd("game_landingairlw", landingairlw, Priority::Low);
    agent.game_acmd("game_throwb", throwb, Priority::Low);
    agent.game_acmd("game_escapeairslide", escapeairslide, Priority::Low);
    agent.game_acmd("game_stepjump", stepjump, Priority::Low);
}