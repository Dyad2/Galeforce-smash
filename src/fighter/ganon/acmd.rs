...
unsafe extern "C" fn attackairhi(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
        ...
        frame(lua_state, 56.);
            if macros::is_excute(fighter)
            {
                notify_event_msc_cmd!(fighter, Hash40::new_raw(0x2127e37c07), *GROUND_CLIFF_CHECK_KIND_ALWAYS_BOTH_SIDES as i32);
            }
}
...
pub fn install(agent: &mut smashline::Agent) {
    ...
    agent.game_acmd("game_throwlw", throwlw, Priority::Low);
    agent.game_acmd("game_escapen", escapen, Priority::Low);
