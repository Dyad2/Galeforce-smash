...
unsafe extern "C" fn sheik_galeforce_attack_weak_attackairlw(fighter: &mut L2CFighterCommon) {
    ...
    if curr_motion_kind == hash40("attack_air_lw") {
        if fighter.global_table[MOTION_FRAME].get_i32() < 5 {
            if VarModule::is_flag(fighter.module_accessor, commons::instance::flag::GALEFORCE_ATTACK_ON) && ControlModule::check_button_off(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) {
                VarModule::on_flag(fighter.module_accessor, sheik::instance::flag::ATTACK_AIR_LW_W);
            }
            else {
                VarModule::off_flag(fighter.module_accessor, sheik::instance::flag::ATTACK_AIR_LW_W);
            }
        }
    }
