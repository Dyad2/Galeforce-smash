...
unsafe fn specialn(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 1.);
        if macros::is_excute(fighter)
        {
            ArticleModule::generate_article(boma, *FIGHTER_PFUSHIGISOU_GENERATE_ARTICLE_SEED, true, 0);
...
unsafe fn specialnend(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 1.);
        if macros::is_excute(fighter)
        {
            ArticleModule::generate_article(boma, *FIGHTER_PFUSHIGISOU_GENERATE_ARTICLE_SEED, false, 0);
        }
    frame(lua_state, 4.);
        if macros::is_excute(fighter)
        {
