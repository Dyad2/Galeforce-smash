...
unsafe fn speciallw3(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 6.);
        if macros::is_excute(fighter)
        {
            ArticleModule::generate_article(boma, *FIGHTER_BRAVE_GENERATE_ARTICLE_FLASH, false, 0);
...
unsafe fn speciallw4(fighter: &mut L2CAgentBase) {
    ...
    let boma = sv_system::battle_object_module_accessor(lua_state);
        ...
        frame(lua_state, 6.);
            if macros::is_excute(fighter)
            {
                ArticleModule::generate_article(boma, *FIGHTER_BRAVE_GENERATE_ARTICLE_EXPLOSION, false, 0);
                WorkModule::on_flag(boma, *FIGHTER_BRAVE_STATUS_SPECIAL_LW_FLAG_ENABLE_CONTROL_ENERGY);
            }
}
