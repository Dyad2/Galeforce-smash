...
fn mini_rat_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if curr_motion_kind == hash40("attack_11") && AttackModule::is_infliction_status(boma, *COLLISION_KIND_MASK_HIT) && MotionModule::frame(boma) >= 7. {
            if ControlModule::get_stick_x(boma).abs() > 0.25 {
                StatusModule::change_status_force(boma, *FIGHTER_STATUS_KIND_DASH, false);
            }
        }
    ...
    wait(lua_state, 3.);
        if macros::is_excute(fighter)
        {
            ...
            if !AttackModule::is_infliction_status(boma, *COLLISION_KIND_MASK_HIT) {
                MotionModule::set_rate(boma, 0.70);
            }
        }
    frame(lua_state, 18.);
        if macros::is_excute(fighter)
...
unsafe fn specialhi1(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = sv_system::battle_object_module_accessor(lua_state);
        ...
        if macros::is_excute(fighter)
        {
            ...
            macros::ATTACK(fighter, 0, 0, Hash40::new("neck"), 2.0, 0, 0, 0, 0, 1.6, 0.0, 0.0, 0.0, None, None, None, 0.0, 1.0, *ATTACK_SETOFF_KIND_THRU, *ATTACK_LR_CHECK_POS, false, 5, 0.0, 0, false, false, true, true, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false,Hash40::new("collision_attr_elec"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_ELEC, *ATTACK_REGION_BODY);
...
unsafe fn specialhi2(fighter: &mut L2CAgentBase) {
    ...
    let boma = sv_system::battle_object_module_accessor(lua_state);
        ...
        {
            DamageModule::add_damage(boma, 0.9, 0);
            macros::ATTACK(fighter, 0, 0, Hash40::new("top"), 2.0, 0, 1, 0, 1, 1.6, 0.0, 0.0, 0.0, None, None, None, 0.0, 1.0, *ATTACK_SETOFF_KIND_THRU, *ATTACK_LR_CHECK_POS, false, 5, 0.0, 0, false, false, true, true, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false,Hash40::new("collision_attr_elec"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_ELEC, *ATTACK_REGION_BODY);
            JostleModule::set_status(boma, false);
        }
}

#[acmd_script( agent = "pichu", script = "game_specialairhi1", category = ACMD_GAME, low_priority)]
unsafe fn specialairhi1(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = sv_system::battle_object_module_accessor(lua_state);
