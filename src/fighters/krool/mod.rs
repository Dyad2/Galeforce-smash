...
fn krool_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        // if VisibilityModule::is_visible_mesh(boma, hash40("crown") as u32) {
        //     FIGHTER_GLOBALS[entry_id as usize].krool_has_crown = true;
        // }
        // else {
        //     FIGHTER_GLOBALS[entry_id as usize].krool_has_crown = false;
        // }
        // if FIGHTER_GLOBALS[entry_id as usize].krool_has_crown {
...
unsafe fn attackhi4(fighter: &mut L2CAgentBase) {
    ...
    frame(lua_state, 12.);
        if macros::is_excute(fighter)
        {
            HitModule::set_status_all(boma, smash::app::HitStatus(*HIT_STATUS_NORMAL as i32), 0);
            AttackModule::clear_all(boma);
        }
        frame(lua_state, 17.);
        if macros::is_excute(fighter)
