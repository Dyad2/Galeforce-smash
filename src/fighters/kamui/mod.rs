...
unsafe fn ryusensya_min(weapon: &mut L2CAgentBase) {
    ...
    let boma = sv_system::battle_object_module_accessor(lua_state);
        ...
        if macros::is_excute(weapon)
        {
            ...
            macros::ATK_LERP_RATIO(weapon, WorkModule::get_float(boma, *WEAPON_KAMUI_RYUSENSYA_INSTANCE_WORK_ID_FLOAT_HOLD_RATE));
...
unsafe fn attackairn(fighter: &mut L2CAgentBase) {
    ...
    let boma = sv_system::battle_object_module_accessor(lua_state);
        ...
        if macros::is_excute(fighter)
        {
            ArticleModule::generate_article(boma, *FIGHTER_KAMUI_GENERATE_ARTICLE_SPEARHAND as i32, false, 0);
            ArticleModule::change_motion(boma, *FIGHTER_KAMUI_GENERATE_ARTICLE_SPEARHAND, smash::phx::Hash40{hash: hash40("attack_air_n")}, false, 0.0);
        }
    frame(lua_state, 5.);
        if macros::is_excute(fighter)
        {
...
unsafe fn specialswallattackb(fighter: &mut L2CAgentBase) {
    ...
    let boma = sv_system::battle_object_module_accessor(lua_state);
        ...
        if macros::is_excute(fighter)
        {
            ArticleModule::change_motion(boma, *FIGHTER_KAMUI_GENERATE_ARTICLE_SPEARHAND, Hash40::new("special_s_wall_attack_b"), false, 0.0);
        }
    frame(lua_state, 6.);
        if macros::is_excute(fighter)
        {
