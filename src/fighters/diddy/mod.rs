...
fn diddy_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if curr_motion_kind == hash40("attack_12") && MotionModule::frame(fighter.module_accessor) >= 7.0 && ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) {
...
#[acmd_script( agent = "diddy", scripts = ["game_attacks3lw", "game_attacks3", "game_attacks3hi"], category = ACMD_GAME, low_priority)]
