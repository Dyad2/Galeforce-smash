...
unsafe extern "C" fn roy_galeforce_attack(fighter: &mut L2CFighterCommon) {
    ...
    if [*FIGHTER_STATUS_KIND_ATTACK, *FIGHTER_STATUS_KIND_ATTACK_DASH, *FIGHTER_STATUS_KIND_ATTACK_HI3, *FIGHTER_STATUS_KIND_ATTACK_LW3, *FIGHTER_STATUS_KIND_ATTACK_S3].contains(&status_kind) {
        if AttackModule::is_infliction(fighter.module_accessor, *COLLISION_KIND_MASK_HIT) {
            VarModule::set_float(fighter.module_accessor, roy::instance::float::GALEFORCE_ATTACK_TIMER, MotionModule::frame(fighter.module_accessor));
        ...
        if ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_APPEAL_HI) {
            if VarModule::is_flag(fighter.module_accessor, commons::instance::flag::GALEFORCE_ATTACK_ON) && MotionModule::frame(fighter.module_accessor) >= VarModule::get_float(fighter.module_accessor, roy::instance::float::GALEFORCE_ATTACK_TIMER) + 10.0 {
                ...
                VarModule::set_float(fighter.module_accessor, roy::instance::float::GALEFORCE_ATTACK_TIMER, 999.0);
            }
        }
    }
