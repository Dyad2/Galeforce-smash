...
pub fn run(fighter : &mut L2CFighterCommon) {
    unsafe {
        ...
        if is_status_damage(&mut *fighter.module_accessor) {
            VarModule::set_int(fighter.module_accessor, commons::instance::int::SUBSTICK_AIR_ATTACK, 0);
        }
        ...
        if ![*FIGHTER_STATUS_KIND_SQUAT_B].contains(&prev_status_kind) {
            if curr_motion_kind == hash40("attack_lw3") && curr_motion_kind != hash40("squat_b") && !AttackModule::is_attack(fighter.module_accessor, 0, false) {
                ...
                && VarModule::is_flag(fighter.module_accessor, commons::instance::flag::ALLOW_REVERSE_ATTACK_LW3)  {
                    if fighter.global_table[MOTION_FRAME].get_i32() <= 3 {
                        VarModule::off_flag(fighter.module_accessor, commons::instance::flag::ALLOW_REVERSE_ATTACK_LW3);
                ...
                VarModule::on_flag(fighter.module_accessor, commons::instance::flag::ALLOW_REVERSE_ATTACK_LW3);
            }
        }
        ...
        if situation_kind == SITUATION_KIND_AIR {
            ...
            if WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_FRAME_IN_AIR) > 12 //because get_stick_flick runs when fighters are on the ground (of course) we don't want to forbid aerial turn if a player does a rar or something
             && (VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME) > 12 || VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_COUNT) > 0) {
                VarModule::on_flag(fighter.module_accessor, commons::instance::flag::AIR_TURN_INVALID);
            }
            else {
                VarModule::off_flag(fighter.module_accessor, commons::instance::flag::AIR_TURN_INVALID);
            }

            if !VarModule::is_flag(fighter.module_accessor, commons::instance::flag::AIR_TURN_INVALID) {
                //checks for input method
                if VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD) == 0 {
                    if ControlModule::check_button_off(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) {
                        ...
                        if VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME) != 0 && ControlModule::get_flick_x(fighter.module_accessor) < 1 && stick_x * PostureModule::lr(fighter.module_accessor) < -0.5 {
                            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD, 1) ;
                        }
                        ...
                        if (stick_x * PostureModule::lr(fighter.module_accessor)) < -0.5 {
                            //VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD, 1) ;
                            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME, 1);
                        } 
                        //turn with taunt
                        if ControlModule::check_button_trigger(fighter.module_accessor, *CONTROL_PAD_BUTTON_APPEAL_S_L) && PostureModule::lr(fighter.module_accessor) > 0.0
                         || ControlModule::check_button_trigger(fighter.module_accessor, *CONTROL_PAD_BUTTON_APPEAL_S_R) && PostureModule::lr(fighter.module_accessor) < 0.0 {
                            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD, 2);
                            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME, 1);
                        }
                    }
                }
                //turn here
                else if VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD) == 1
                 || (VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD) == 2 && VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME) > 4) { //method check
                    if WorkModule::is_enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ATTACK_AIR) && MotionModule::end_frame(fighter.module_accessor) - fighter.global_table[MOTION_FRAME].get_f32() > 5.0 {
                        if !(fighter_kind == *FIGHTER_KIND_PICKEL && ([*FIGHTER_PICKEL_STATUS_KIND_ATTACK_FALL, *FIGHTER_PICKEL_STATUS_KIND_ATTACK_FALL_AERIAL, *FIGHTER_PICKEL_STATUS_KIND_ATTACK_JUMP, *FIGHTER_PICKEL_STATUS_KIND_ATTACK_JUMP_AERIAL, *FIGHTER_PICKEL_STATUS_KIND_ATTACK_JUMP_SQUAT].contains(&status_kind))) {
                            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD, 0) ;
                            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME, 0);
                            VarModule::add_int(fighter.module_accessor, commons::instance::int::AIR_TURN_COUNT, 1);
                            PostureModule::reverse_lr(fighter.module_accessor);
                            PostureModule::update_rot_y_lr(fighter.module_accessor);
                ...
                if VarModule::get_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME) != 0 { //if we enter here it means we initiated either input method
                    VarModule::add_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME, 1);
                }
            }
        ...
        //not in the air, reset
            ...
            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INITIATE_METHOD, 0) ;
            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_INPUT_FRAME, 0);
            VarModule::set_int(fighter.module_accessor, commons::instance::int::AIR_TURN_COUNT, 0);
        }
