...
fn tortue_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if [*FIGHTER_PZENIGAME_STATUS_KIND_SPECIAL_N_CHARGE, *FIGHTER_PZENIGAME_STATUS_KIND_SPECIAL_N_SHOOT].contains(&status_kind) && MotionModule::frame(fighter.module_accessor) <= 4.0 {
