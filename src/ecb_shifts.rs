...
//=================================================================
pub unsafe fn ecb_shifts(fighter : &mut L2CFighterCommon) {
    ...
    let vanilla_ecb = ([
                    ...
                    || (WorkModule::is_flag(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLAG_GANON_SPECIAL_S_DAMAGE_FALL_AIR | *FIGHTER_INSTANCE_WORK_ID_FLAG_GANON_SPECIAL_S_DAMAGE_FALL_GROUND))
                    || (VarModule::is_flag(fighter.battle_object, commons::instance::flag::ESCAPE_AIR_UP));
    ...
    if is_fighter(&mut *fighter.module_accessor) {
        ...
        if situation_kind == *SITUATION_KIND_AIR {
            if WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_FRAME_IN_AIR) > 10 {
                ...
                if object_kind == *FIGHTER_KIND_LUCINA && [hash40("special_hi_3"), hash40("landing_fall_special")].contains(&curr_motion_kind) {
                    max_offset = 0.;
                }
            }
        }
        else if situation_kind == *SITUATION_KIND_GROUND {
            VarModule::off_flag(fighter.battle_object, commons::instance::flag::ESCAPE_AIR_UP);
            max_offset = 0.;
            VarModule::set_float(fighter.battle_object, commons::instance::float::ECB_OFFSET_Y, 0.0);
        } 
        else {
